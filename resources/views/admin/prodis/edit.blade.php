...
<div class="max-w-2xl">
    <div class="bg-white rounded-xl border border-gray-100 overflow-hidden">
        <div class="p-6">
            <form action="{{ route('admin.prodis.update', $prodi) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')

                <div>
                    <label for="name" class="block text-sm font-semibold text-gray-700 mb-1.5">Nama Prodi <span class="text-red-500">*</span></label>
                    <input type="text" name="name" id="name" value="{{ old('name', $prodi->name) }}" required
                ...
                <div>
                    ...
                    <select name="jurusan" id="jurusan" required
                        ...
                        @foreach ($jurusans as $j)
                            <option value="{{ $j }}" {{ old('jurusan', $prodi->jurusan) === $j ? 'selected' : '' }}>{{ $j }}</option>
                        @endforeach
                    </select>
                </div>

                <div class="flex items-center gap-3">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $prodi->is_active) ? 'checked' : '' }}
                ...
                <div class="flex items-center gap-3 pt-4 border-t border-gray-100">
                    ...
                    <button type="submit" class="px-6 py-2.5 rounded-xl text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-200">Perbarui</button>
                </div>
            </form>
        </div>
    </div>
