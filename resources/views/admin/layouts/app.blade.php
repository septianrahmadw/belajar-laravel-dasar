    ...
    <div class="flex min-h-screen">
        <aside id="sidebar" class="bg-gray-900 text-white flex flex-col shrink-0 fixed h-full z-40 overflow-hidden">
            <div class="p-5 border-b border-gray-800 flex items-center gap-2.5">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-2.5 min-w-0">
                    ...
                    <div class="sidebar-header-text whitespace-nowrap overflow-hidden">
                        <span class="text-lg font-bold">Lab<span class="text-blue-400">Booking</span></span>
            ...
            <nav class="flex-1 p-3 space-y-1">
                <a href="{{ route('admin.dashboard') }}" class="nav-link relative flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors {{ request()->routeIs('admin.dashboard') ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white' }}">
                ...
                <a href="{{ route('admin.rooms.index') }}" class="nav-link relative flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors {{ request()->routeIs('admin.rooms.*') ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white' }}">
                ...
                <a href="{{ route('admin.bookings.index') }}" class="nav-link relative flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors {{ request()->routeIs('admin.bookings.*') ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white' }}">
                ...
                <a href="{{ route('admin.schedule') }}" class="nav-link relative flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors {{ request()->routeIs('admin.schedule*') ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white' }}">
                    <svg class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5m-9 3h1.5m-3 0h3M9 12.75h.008v.008H9v-.008zM12 12.75h.008v.008H12v-.008zM15 12.75h.008v.008H15v-.008z" /></svg>
                    <span class="sidebar-label">Jadwal Ruangan</span>
                    <span class="nav-tooltip absolute left-full ml-2 px-2 py-1 bg-gray-800 text-white text-xs rounded-md whitespace-nowrap shadow-lg z-50">Jadwal Ruangan</span>
                </a>

                <a href="{{ route('admin.prodis.index') }}" class="nav-link relative flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors {{ request()->routeIs('admin.prodis.*') ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white' }}">
                ...
                <a href="{{ route('admin.users.index') }}" class="nav-link relative flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors {{ request()->routeIs('admin.users.*') ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white' }}">
            ...
            <div class="p-3 border-t border-gray-800">
                ...
                <div class="flex items-center gap-3 px-3 py-2">
                    <div class="w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-semibold shrink-0
                        {{ auth()->user()->isAdmin() ? 'bg-blue-600' : 'bg-amber-600' }}">
                    ...
                    <div class="user-info flex-1 min-w-0 whitespace-nowrap overflow-hidden">
                        ...
                        <p class="text-[11px] text-gray-500 truncate flex items-center gap-1">
                            <span class="inline-flex items-center px-1.5 py-0 rounded text-[9px] font-bold uppercase
                                {{ auth()->user()->isAdmin() ? 'bg-blue-900 text-blue-300' : 'bg-amber-900 text-amber-300' }}">
            ...
            <header class="bg-white border-b border-gray-200 sticky top-0 z-30">
                <div class="flex items-center justify-between h-16 px-8">
                    <div class="flex items-center gap-3">
                        <button id="sidebarToggle" type="button" class="p-2 rounded-lg text-gray-500 hover:bg-gray-100 hover:text-gray-700 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2" title="Toggle Sidebar">
